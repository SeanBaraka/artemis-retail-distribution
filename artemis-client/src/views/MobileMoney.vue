    ...
    getPayments()
      .then((resp) => {
        for (var i = 0; i < resp.payments.length; i++) {
          ...
          resp.payments[
            i
          ].Name = `${resp.payments[i].FirstName} ${resp.payments[i].MiddleName} ${resp.payments[i].LastName}`;
          resp.payments[i].TransAmount = Number(
            resp.payments[i].TransAmount
          ).toFixed(2);
